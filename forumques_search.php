...
<body>
    ...
    if (isset($_SESSION["title"])) {
        ...
        $search_title = $conn->query("SELECT article.*,member.* FROM article,member WHERE member.m_Id=article.m_Id and type_id=1 and title LIKE '%$title%'");
            ...
            <div class="col-11 pt-5 pl-5 pr-5 m-0">
                ...
                <?php
                    if(isset($_GET['seach']) and $_GET['seach']=='success'){
                        $title=$_SESSION["title"];
                        $carti = $conn->prepare("SELECT * FROM `article` WHERE title like '%$title%' and type_id=1");
                        $carti->execute();
                ...
                <?php  
                    while (!empty($row)) { 
                    if (($row['m_Id']!='manager1' and !empty($_SESSION['acc']) and $_SESSION['acc']!=$row['m_Id'] and isset($_GET['seach']) and $_GET['seach'] == 'success')) {
                ?>
                ...
                } elseif (($row['m_Id']!='manager1' and !empty($_SESSION['acc']) and $_SESSION['acc']==$row['m_Id'] and isset($_GET['seach']) and $_GET['seach'] == 'success')) { 
                ...
                ?>
                        ...
                        <br>
                    <?php
                } elseif (($row['m_Id']=='manager1' and isset($_GET['seach']) and $_GET['seach'] == 'success')) { 
                    ?>
                            <div class="p-3 rounded" id="showforum" onclick="location.href='forumexper.php?msg=edit&id=<?php echo $row['arti_Id']; ?>';" style="background-color: rgb(239, 239, 239);">
                                <div class="d-flex">
                                    <div style="display:block; padding-left:-30px;">
