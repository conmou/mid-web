...
switch ($action) {
    case "edit":
        $todos = $conn->prepare("SELECT article.*,member.img_name,member.m_Id FROM article,member WHERE member.m_Id=article.m_Id and arti_Id=:id");
        ...
        if(!empty($_SESSION['acc'])){
            $img = $conn->prepare("SELECT member.img_name FROM article,member WHERE member.m_Id=:mid and arti_Id=:id");
            //$todos->bindValue(':id',$id);
            $img->execute(["mid" => $member_img , "id" => $id]);
            $mem_img = $img->fetch();
            $com_img=$mem_img['img_name'];
        }
