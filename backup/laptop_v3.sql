...
INSERT INTO `article` (`m_Id`, `arti_Id`, `title`, `content`, `post_time`, `type_id`) VALUES
('conmou0225', 1, 'å˜¿å˜¿å˜¿', 'å“ˆå“ˆå“ˆ\r\nã„šã„šã„š', '2022-05-15 23:25:21', 1),
('conmou0225', 2, 'ㄚㄚㄚㄚ', '你啊\r\n啊哈', '2022-05-16 10:15:18', 1),
('conmou0225', 3, '#Mac #請益 MacBook Pro 14吋', '想請問各位板上的前輩～因為之前人生第一台MacBook 是到apple 店裡直接購買剛剛收到法雅客的電話 說有現貨了已在apple 官網訂購 但要等到7/8想請問有買過的前輩們覺得可以到法雅客直接購買嗎？會有什麼差異嗎～還是應該乖乖等官網呢？（但是因為9月要出國 怕又被延後 無法在9月前收到?）#補充一下 我是買標配～～謝謝各位', '2022-05-16 10:40:22', 1),
('conmou0225', 4, 'aaaaa', 'bbbbbbbbbbbbbbb', '2022-05-16 11:36:45', 1),
('ejiscutela', 5, '你在幹嘛拉拉拉拉拉', '哈哈哈哈哈哈哈哈哈哈哈哈哈哈', '2022-05-17 10:50:44', 1),
('conmou0225', 22, '業務選購筆電(內有選手)', '各位好，小弟目前為業務員<br>\r\n                            公司雖然有筆電，但實在太陽春<br>\r\n                            想說為了工作需求還是買一台筆電比較方便<br>\r\n                            因沒買過筆電，最近才在做功課<br>\r\n                            還請多多指點……', '2022-05-18 23:56:47', 2),
('plum9241', 23, 'Razer雷蛇 Pro Click 無線/藍芽雙模無線滑鼠開箱', '最近疫情影響周邊很多朋友尤其是因為(同事或小孩同學)確診而需要在家照顧家人或小孩的人\r\n\r\n簡直就只有一個想法就是: 歸剛耶\r\n\r\n所以為了減少不不要接觸最近又開始WFH\r\n在家專心操自己的鍵盤滑鼠 使用時間又間接變多了\r\n\r\n很不巧的偏偏家裡滑鼠不受控 最常使用於工作用的羅技滑鼠故障了\r\n\r\n阿就上網看看最近什麼優惠或是造型好看的 中階文書處理用滑鼠\r\n用膩了羅技的滑鼠還有一堆RGB快樂改版的滑鼠 現在想改找簡單設計也符合筆電風格的\r\n\r\n我一直以來沒有買過雷蛇商品\r\n就在前幾天看到其創辦人的社群推文後\r\n就來逛逛購物平台有什麼好看的\r\n\r\n所以 不到兩千元入門雷蛇Razer Pro Click 無線/藍芽雙模無線滑鼠開箱文章登場\r\n\r\n除了想入手雷蛇玩玩以外\r\n現在網購還送一張Xbox遊戲體驗券 但基本上我用不到 丟掉!', '2022-05-20 00:33:40', 2),
('ejiscutela', 24, '大飛darkFlash Twister DX v2 360mm一體式水冷散熱器-炎炎夏日散熱升級好夥伴', '大飛darkFlash Twister DX v2 360mm一體式水冷散熱器表現非常出色，搭配超頻後Intel Core i9-12900K依然展現強悍的壓制力，燒機過程約93.3~106.8度，最高溫度為110度，平均CPU功率來到319.96W，要解掉300W以上功耗非常輕鬆，風扇因為全轉的關係稍微有風切聲，在可接受範圍內；視覺燈效方面冷頭與風扇都擁有無限鏡面效果，還內建了多款視覺燈效，整體可提供非常不錯的視覺饗宴；想找表現散熱表現出色且擁有無限鏡面視覺饗宴的ARGB一體式水冷產品，大飛darkFlash Twister DX v2一體式水冷散熱器是款不錯的新選擇，這款產品會在六月底上市販售，玩家們可以期待一下喔。', '2022-05-20 00:33:40', 2),
('ejiscutela', 25, 'test1', '123456789', '2022-05-22 14:59:43', 1),
('ejiscutela', 26, 'test2', '12345789', '2022-05-22 15:00:01', 2),
('ejiscutela', 27, '123456789', '987654321', '2022-05-22 15:00:16', 1);
...
INSERT INTO `forum` (`type_Id`, `type_name`) VALUES
(1, '選購請益'),
(2, '使用心得');
...
--
-- 資料表新增資料前，先清除舊資料 `message_board`
--

TRUNCATE TABLE `message_board`;
--
-- 傾印資料表的資料 `message_board`
--

INSERT INTO `message_board` (`msg_Id`, `arti_id`, `m_id`, `content`, `post_time`) VALUES
(6, 3, 'conmou0225', '請問大大', '2022-05-20 12:02:27'),
(7, 3, 'conmou0225', '好棒好棒', '2022-05-20 12:06:52');
...
DROP VIEW IF EXISTS `all_platform`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `all_platform`  AS SELECT `specifics_yahoo`.`nb_id` AS `nb_id`, `specifics_yahoo`.`yahoo_price` AS `yahoo_price`, `specifics_yahoo`.`yahoo_url` AS `yahoo_url`, `sinya`.`sinya_price` AS `sinya_price`, `sinya`.`sinya_url` AS `sinya_url`, `sinya_stu`.`sinya_stu_price` AS `sinya_stu_price`, `pchome`.`pchome_url` AS `pchome_url`, `pchome`.`pchome_price` AS `pchome_price` FROM (((`specifics_yahoo` left join `sinya` on(`sinya`.`nb_id` like concat('%',`specifics_yahoo`.`nb_id`,'%'))) left join `sinya_stu` on(`sinya`.`sinya_url` = `sinya_stu`.`sinya_stu_url`)) left join `pchome` on(`pchome`.`nb_id` like concat('%',`specifics_yahoo`.`nb_id`,'%'))) ;
...
DROP VIEW IF EXISTS `yahoo_pchome`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `yahoo_pchome`  AS SELECT `specifics_yahoo`.`nb_id` AS `nb_id`, `specifics_yahoo`.`yahoo_price` AS `yahoo_price`, `specifics_yahoo`.`yahoo_url` AS `yahoo_url`, `pchome`.`pchome_price` AS `pchome_price`, `pchome`.`pchome_url` AS `pchome_url` FROM (`specifics_yahoo` join `pchome` on(`pchome`.`nb_id` like concat('%',`specifics_yahoo`.`nb_id`,'%'))) ;
...
DROP VIEW IF EXISTS `yahoo_sinya`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `yahoo_sinya`  AS SELECT `specifics_yahoo`.`nb_id` AS `nb_id`, `specifics_yahoo`.`yahoo_price` AS `yahoo_price`, `specifics_yahoo`.`yahoo_url` AS `yahoo_url`, `sinya`.`sinya_price` AS `sinya_price`, `sinya_stu`.`sinya_stu_price` AS `sinya_stu_price`, `sinya`.`sinya_url` AS `sinya_url` FROM ((`specifics_yahoo` left join `sinya` on(`sinya`.`nb_id` like concat('%',`specifics_yahoo`.`nb_id`,'%'))) join `sinya_stu` on(`sinya`.`sinya_url` = `sinya_stu`.`sinya_stu_url`)) ;
...
--
ALTER TABLE `article`
  MODIFY `arti_Id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;
...
--
ALTER TABLE `article`
  ADD CONSTRAINT `article_ibfk_1` FOREIGN KEY (`type_id`) REFERENCES `forum` (`type_Id`);
