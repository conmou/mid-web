...
<body>
    ...
    $todos = $conn->query("SELECT article.*,member.img_name FROM article,member WHERE member.m_Id=article.m_Id ORDER BY article.arti_Id=54 DESC,article.post_time DESC");
    //$msg = $conn->query("SELECT * FROM message_board WHERE arti_id= ORDER BY post_time ASC");
    $astate = $conn->query("SELECT * FROM article");
    $asta=$astate->fetch();
    $cstate = $conn->query("SELECT * FROM message_board");
    $csta=$cstate->fetch();
    ?>
        ...
        <div class="row m-0 p-0" style="height:1000px; margin:0;">
            <div class="col text-dark m-0 p-0" style="background-color: #cccccc;">
                <div class="btn-group-vertical float-right h-200" role="group" style="padding-top: 100%!important;" aria-label="Button group with nested dropdown">
                    <div>
                        <a href="http://localhost:8080/mid-web/forumques.php"><button type="button" id="vertical" class="btn btn-secondary" style="background-color: #fff;">選購請益</button></a>
                    </div>
                    <div>
            ...
            <div class="col-11 pt-5 pl-5 pr-5 m-0">
                <div class="col-11 d-flex">
                    <h1 class="m-0 col-8" style="font-size: 50px; color:#44546A; font-weight:bold;">選購請益</h1>
                    <div class="col-5 mt-3">
                        <form action="forum_app/ques_search.php" method="POST" autocomplete="off">
                ...
                if(!isset($_GET['msg']) and !isset($_GET['action'])){
                    $carti = $conn->prepare("SELECT * FROM `article` WHERE type_id=1");
                    $carti->execute();
                ...
                if (isset($_GET['msg'])) {
                    require 'forum_app/ques_msg.php';
                    if (!empty($_SESSION['acc']) and $todo['m_Id']!=$_SESSION['acc'] and $todo['arti_Id']!=54){
                ?>
                    ...
                    <br>
                    <?php }elseif(!empty($_SESSION['acc']) and $todo['m_Id']==$_SESSION['acc'] and $todo['arti_Id']!=54){ ?>
                        <div class="p-3 rounded" style="background-color: rgb(239, 239, 239);border-radius: 15px !important;box-shadow: 0 2px 2px 1px #ccc, 0 2px 2px 1px #ccc;">
                            <a href="forumques.php?action=edit&id=<?php echo $todo['arti_Id']; ?>" id="editBtn" style="width: 50px; font-size: 0.1px;">●&nbsp&nbsp●&nbsp&nbsp●</a>
                            <div class="d-flex">
                                <div style="display:block; padding-left:-30px;">
                                    <?php
                                    if (!empty($m_img)) {
                    ...
                    <?php if (isset($_SESSION['acc'])) { ?>
                        <div class="p-3" id="msg" style="background-color: rgb(255, 255, 255);border-top:2px solid #44546A">
                            ...
                            <form action="forum_app/ques_msg.php?msg=insert&id=<?php echo $id ?>&member=<?php echo $_SESSION['acc']; ?>" method="POST" autocomplete="off">
                ...
                <?php while ($todo = $todos->fetch(PDO::FETCH_ASSOC)) {
                    if($todo['state']==0){
                    if( $todo['arti_Id'] == 54 and $acc=='manager1' and $todo['m_Id']=='manager1') { ?>
                            ...
                            <br>
                <?php } elseif( $todo['arti_Id'] == 54 and $todo['m_Id']=='manager1' and $acc!='manager1' and !isset($_GET['action'])){ ?>
                            <div class="p-3 rounded" id="showforum" onclick="location.href='forumques.php?msg=edit&id=<?php echo $todo['arti_Id']; ?>';" style="background-color: rgb(239, 239, 239);">
                                <div class="d-flex">
                                    <div style="display:block; padding-left:-30px;">
                                        <?php
                                    ...
                                    <h3 class="p-3" style="color: #44546A;font-weight: bold;"><?php echo $todo['m_Id']; ?></h3>
                                    <div class="ml-auto">
                                        <i class="bi bi-pin-fill" style="color:#6c757d;"></i>
                                    </div>
                                </div>
                                <div class="p-2" id="">
                                    <h3 style="color: #44546A;font-weight: bold;"><?php echo $todo['title']; ?></h3>
                            ...
                            <br>
                <?php } elseif ($todo['type_id'] == 1 ) {
                        if (isset($_GET['action'])) {
                            require 'forum_app/ques_edit.php';
                        ?>
                                ...
                                <div class="p-2" id="content-size">
                                    <span>
                                        <div class="edit-to-do">
                                            <form action="forum_app/ques_edit.php?action=update&id=<?php echo $id ?>" method="POST" autocomplete="off">
